...
\begin{figure}[H]
  ...
  \caption{Comparison of different activation functions. Note that the: linear, ReLU, LeakyReLU and ELU activations coninside for $x > 0$.}
...
\begin{equation}
\mu_{\text{running}} \leftarrow (1 - \lambda),\mu_{\text{running}} + \lambda,\mu_{\text{batch}},
\end{equation}

\begin{equation}
\sigma^2_{\text{running}} \leftarrow (1 - \lambda),\sigma^2_{\text{running}} + \lambda,\sigma^2_{\text{batch}},
\end{equation}
...
\paragraph{Simple convolutional block}
...
\paragraph{Squeeze and excitation block}
