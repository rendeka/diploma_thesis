...
The field of \emph{machine learning (ML)} is a complex discipline grounded in the language of probability theory, statistics, and information theory \cite{bengio2017deep}. In this chapter, we describe all the methods and ML techniques applied in the practical part of this thesis. However, to understand all the methods, one must be familiar in the basic concepts from machine learning. Focusing on the applied methods in this chapter, we include the definitions and descriptions of ML fundamentals in the appendix of this thesis \ref{sec:ML-background}.
...
The general structure has an \emph{input layer}, several \emph{hidden layers}, and an \emph{output layer}. Each layer transforms its input vector into an output vector through a combination of a linear transformation and a non-linear activation function.
...
Between the convolutional features and the final output layer, it is usually beneficial to include a few fully connected layers. This additional structure helps the model better combine the extracted spatial features before producing the final prediction. The densely connected section on top convolutional layers always have the following structure in our implementations:

\begin{itemize}
    \item \textbf{Dense} layer: combines extracted spatial features from the convolutional backbone.
    \item \textbf{Dropout}: regularization (we defaultly turn it off).
...
\emph{RMSProp} \cite{hinton2012rmsprop} was designed to address slow convergence by using adaptive learning rate. RMSProp does it by maintaining an exponentially decaying average of squared gradients:
...
In \emph{linear decay}, the learning rate is reduced linearly from its initial value to a predefined minimum value over the course of training. It is one of the simplest decay strategies. Suppose that $l_0$ is the initial learning rate and $T$ is the total number of steps, then the learning rate at step $t$ is:
...
A \emph{weight decay} is very similar to $L^2$ regularization, but is implemented little differently. Instead of changing the loss functio
...
Yet this is not the only benefit data augmentation brings for our usecase. We can also combine images from different classes in order to help our model recognize phase transitions. There are two common approaches for such data augmentation combining multiple classes, \emph{CutMix} \cite{yun2019cutmix} and \emph{MixUp} \cite{zhang2017mixup}. CutMix works by taking a portion of image $1$ (usually rectangular segment) and pasting it onto an image $2$. Resulting image will get label that is a weighted sum of original one-hot encoded labels, with weights proportional to number of pixels contributed by each image. MixUp works again by taking two images, generating random number $\alpha \in (0, 1)$ and adding pixels of these images weighted by $\alpha$ (resp. $1 - \alpha$). Label is then created in same manner. Examples of these augmentations on our data are in \cref{fig:aug_fe,fig:aug_sp}
