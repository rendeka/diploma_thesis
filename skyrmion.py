...
parser.add_argument("--augment", default="custom", type=str, choices=["None", "cutmix", "mixup", "adaptive", "custom"], nargs="+", help="Augmentation type")
...
def main(args: argparse.Namespace) -> None:
    ...
    def augmented_collate_fn(examples, augment=args.augment):
        ...
        # Choose batch augmentation
        # batch_aug = choose_augmentation(labels)

        batch_aug, labels = choose_augmentation(labels, augment)

        # images, labels = batch_aug(images.permute(0, 3, 1, 2), torch.argmax(labels, dim=1)) # why again to sparse representation?
        images, labels = batch_aug(images.permute(0, 3, 1, 2), labels) # trying one-hot representation
        # #########
